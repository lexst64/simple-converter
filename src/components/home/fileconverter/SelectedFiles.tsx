...
export default function SelectedFile({ fileName, fileSize, onFileFormatChange, outFormats, isValid, isUploading, errorMessage, onFileRemove: onRemove }: SelectedFileProps) {
        ...
        <div className="selected-file-controls">
                    <label>
                        ...
                        >
                            ...
                            <option value=''>---</option>
                            {/* supported formats */}
                            {outFormats.map((item, i) => <option key={i} value={item}>{item}</option>)}
                        </select>
                    </label>
